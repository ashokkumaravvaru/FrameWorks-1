...
public class Test1 extends DriverEngine {
	...
	{
		PropertiesReusabilites p=POJO.getPR(GmailConstants.getConfigPath());
		String url=p.readProperty("url");
		getDriver().get(url);
		GmailHomePage ghp=new GmailHomePage();
		String one = POJO.getER(GmailConstants.getExcelPath()).getCellValue("sheet1", 0, 0);
			
		System.out.println("the data in that one cell is : " +one);
		
		ghp.tex_emailOrPhone(POJO.getER(GmailConstants.getExcelPath()).getCellValue("Sheet1", 0,0));
		ghp.btng_next();
		Thread.sleep(3000);
		
		GmailPaswordPage gpp = new GmailPaswordPage();
		gpp.tex_password(POJO.getER(GmailConstants.getExcelPath()).getCellValue("Sheet1", 0, 1));
		gpp.btng_pwdNext();
